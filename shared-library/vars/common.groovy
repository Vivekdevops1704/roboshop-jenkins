...
def codeQuality() {
  stage('Code Quality') {
    ...
    echo "${sonaruser}"
    echo "${sonarpass}"
    sh 'sonar-scanner -X -Dsonar.host.url=http://54.224.207.41:9000 -Dsonar.login=echo "${sonaruser}" -Dsonar.password=echo "${sonarpass}"-Dsonar.projectKey=${component} -Dsonar.qualitygate.wait=true'
      print 'OK'
   }
}
